...
const actions = (() => {
  ...
  const remove = () => {
    $('.js-remove').on('click', (e) => {
      ...
      swal({
        type: 'warning',
        title: 'Член комманды будет удален безвозвратно. Вы уверены?',
        showCancelButton: true,
        confirmButtonText: 'Удалить',
        cancelButtonText: 'Отмена',
        showLoaderOnConfirm: true,
        preConfirm: () => TeamAPI.remove(id),
  ...
  return {
    init() {
      initModalSwitcher(window.app_data.team);
      imagePreview();
      remove();
    }
  };
  ...
  const initValidationAdd = (rulesOverride = {}) => {
    const $validator = $('#formApi').validate({
      submitHandler(form) {
        ...
        const promise = isEdit ? TeamAPI.edit(id, form, file) : TeamAPI.store(form, file);
        promise
          .then(() => {
            $('#modalApi').modal('hide');
            showSuccess(isEdit);
